...
int est_valide(Monde *mon){
    int i = 0;
    if (mon->serpent.pos[0].x < 25 || mon->serpent.pos[0].x > M * PIXEL + 25 || mon->serpent.pos[0].y < 25 || mon->serpent.pos[0].y > N * PIXEL + 25)
        return 0;
    for (i = 1 ; i < mon->serpent.taille ; ++i){
        if(mon->serpent.pos[0].x == mon->serpent.pos[i].x && mon->serpent.pos[0].y == mon->serpent.pos[i].y)
            return 0;
    }
    return 1;
}
...
int deplacer_serpent(Monde *mon){
    ...
    if(mon->serpent.dir == E){
        ...
        if(!est_valide(mon) || mon->tab_jeu.tab[((y -25) /PIXEL)][((x- 25) /PIXEL) + 1] == 1){
        ...
        if(!est_valide(mon) || mon->tab_jeu.tab[((y -25) /PIXEL) - 1][((x- 25) /PIXEL)] == 1){
